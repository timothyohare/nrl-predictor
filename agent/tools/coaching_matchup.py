...
def get_coaching_matchup(team_a: str, team_b: str, table=None) -> dict:
    """Return coach-vs-coach record between two teams during current coaches' tenures."""
    ...
    if total == 0:
        ...
        rate = round(a_wins / total * 100)
        edge = f"{coach_a['coach']} ({coach_a['team']}) leads {a_wins}-{b_wins} ({rate}% win rate)"
    elif b_wins > a_wins:
        rate = round(b_wins / total * 100)
        edge = f"{coach_b['coach']} ({coach_b['team']}) leads {b_wins}-{a_wins} ({rate}% win rate)"
    else:
