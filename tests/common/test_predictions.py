...
def test_prediction_missing_timestamp_sorts_last():
    """A row without generatedAt must lose to any timestamped row, not crash the sort."""
    preds = [{"predicted_winner": "no-ts"}, _p("2026-06-15T10:00:00+00:00")]
    chosen = latest_before_kickoff(preds, None)
    assert chosen["generatedAt"] == "2026-06-15T10:00:00+00:00"


def test_prediction_at_exact_kickoff_counts_as_pre_ko():
    """The <= boundary: generated exactly at kickoff is still an honest pre-KO prediction."""
    preds = [_p("2026-06-19T10:00:00+00:00"), _p("2026-06-21T11:00:00+00:00")]
    chosen = latest_before_kickoff(preds, KO)  # KO is the same instant as the first pred
    assert chosen["generatedAt"] == "2026-06-19T10:00:00+00:00"
    assert not is_hindsight(chosen, KO)
...
def test_parse_ts_datetime_and_junk_inputs():
    naive = datetime(2026, 6, 19, 10, 0, 0)
    assert parse_ts(naive).tzinfo is UTC          # naive datetime -> assumed UTC
    aware = datetime(2026, 6, 19, 10, 0, 0, tzinfo=UTC)
    assert parse_ts(aware) is aware               # aware datetime passes through
    assert parse_ts(123) is None                  # non-str, non-datetime -> None, not a crash
