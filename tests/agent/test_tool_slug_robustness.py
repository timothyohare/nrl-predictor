...
from v1.agent.tools.coaching_matchup import _get_coach
...
def test_recent_form_resolves_any_form_across_mixed_storage(results_table, name):
    out = get_recent_form(name, table=results_table)
...
def test_head_to_head_resolves_long_names_across_mixed_storage(results_table):
    out = get_head_to_head("Manly Sea Eagles", "Melbourne Storm", table=results_table)
