...
class NrlPredictorStack(cdk.Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        agent_fn = _lambda.Function(
            ...
            runtime=LAMBDA_RUNTIME,
            handler="v1.agent.lambda_handler.lambda_handler",
            code=scraper_code,
            layers=[deps_layer],
        ...
        orchestrator_fn = _lambda.Function(
            ...
            runtime=LAMBDA_RUNTIME,
            handler="v1.orchestrator.lambda_handler.lambda_handler",
            code=scraper_code,
            layers=[deps_layer],
        ...
        retrospective_fn = _lambda.Function(
            ...
            runtime=LAMBDA_RUNTIME,
            handler="v1.retrospective.lambda_handler.lambda_handler",
            code=scraper_code,
            layers=[deps_layer],
            timeout=cdk.Duration.minutes(3),
            memory_size=512,
        ...
        tournament_worker_fn = _lambda.Function(
            ...
            runtime=LAMBDA_RUNTIME,
            handler="v1.tournament.worker_lambda.lambda_handler",
            code=scraper_code,
            layers=[deps_layer],
        ...
        tournament_orchestrator_fn = _lambda.Function(
            ...
            runtime=LAMBDA_RUNTIME,
            handler="v1.tournament.orchestrator_lambda.lambda_handler",
            code=scraper_code,
            layers=[deps_layer],
            timeout=cdk.Duration.minutes(2),
            memory_size=256,
        ...
        tournament_scorer_fn = _lambda.Function(
            ...
            runtime=LAMBDA_RUNTIME,
            handler="v1.tournament.scorer_lambda.lambda_handler",
            code=scraper_code,
            layers=[deps_layer],
            timeout=cdk.Duration.minutes(5),
            memory_size=256,
        ...
        # Sunday 10:00 UTC (Sunday 20:00 AEST) — score tournament variants after Sunday games.
        # Sunday 6pm AEST game ends ~8pm AEST (10:00 UTC); scoring runs at that point
        # so results are available before Monday.
        events.Rule(
            self, "TournamentScorerRule",
            rule_name="nrl-tournament-scorer-sunday",
            schedule=events.Schedule.cron(minute="0", hour="10", week_day="SUN"),
            targets=[targets.LambdaFunction(
