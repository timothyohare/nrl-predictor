...
def lambda_handler(event: dict, context) -> dict:
    ...
    if path.startswith("/predictions"):
        from v1.api.predictions import lambda_handler as predictions_handler
        return predictions_handler(event, context)

    if path == "/accuracy":
        from v1.api.accuracy import lambda_handler as accuracy_handler
        return accuracy_handler(event, context)

    if path == "/tournament/leaderboard":
        from v1.api.tournament import lambda_handler as tournament_handler
